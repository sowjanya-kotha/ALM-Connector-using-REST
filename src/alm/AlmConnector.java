...
/**
	...
	public boolean createSession(String username, String password) throws Exception {
		...
		if(!loginSuccess) {
			throw new Exception ("Unable to login. Please check your credentials");
		}
		
		String sessionUrl = con.buildUrl("rest/site-session");
		/*String sessionUrl = con.buildUrl("api/authentication/sign-in");
...
//		map.put("Authorization", credEncodedString);
		...
		Response response = con.httpPost(sessionUrl, null, map);
		int responseCode = response.getStatusCode();
 
		if (responseCode == HttpURLConnection.HTTP_OK || responseCode == HttpURLConnection.HTTP_CREATED) {
			System.out.println("Succesfully created a session");
		}
		else {
			System.out.println("Unable to create a session. Response code: " + responseCode);
			throw response.getFailure();
		}
